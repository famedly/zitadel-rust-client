...
async fn mk_zitadel_client() -> Result<Zitadel> {
	...
	Zitadel::new(client, url, service_account_file.to_path_buf(), None).await
}

#[tokio::test]
async fn test_e2e_create_token() -> Result<()> {
	assert!(mk_zitadel_client().await.is_ok());
	Ok(())
}
...
async fn test_e2e_simple_token_verification() -> Result<()> {
	...
	let verified_token: Result<token::JwtPayload, _> = token_verifier.verify(token.clone()).await;
...
async fn test_e2e_token_verification_negative() -> Result<()> {
	...
	for jwt in invalid_tokens {
		assert!(token_verifier.verify::<HashMap<String, serde_json::Value>>(jwt).await.is_err());
	}
...
async fn test_e2e_token_verification_positive() -> Result<()> {
	...
	assert_eq!(verification_result?.claims().test_claim, "test_value");
