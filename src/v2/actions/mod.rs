...
impl Zitadel {
	...
	) -> Result<V2betaDeleteTargetResponse> {
		let request =
	...
	) -> Result<impl Stream<Item = Result<V2betaExecution>> + Send + Sync + use<'a>> {
		Ok(futures::stream::try_unfold((0, VecDeque::new()), async |(mut page, mut buffered)| {
